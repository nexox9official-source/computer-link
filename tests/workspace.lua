...
,'')..'/'
    ...
    o.filePath='/user';o.fileOffset=0
    o:openApp('files');o:render()
    local nextButton=nil
    for _,button in ipairs(o.buttons) do
      if button.id=='file:next' then nextButton=button break end
    end
    ...
    fs.list=oldList
    for i=1,14 do disk[string.format('/user/file%02d.txt',i)]=nil end
  end
  for _,app in ipairs({'network','messages','contacts','files','security','settings','calculator','terminal','about'}) do
    o:openApp(app)
    -- All actual built-in renderers run in the mock terminal.
